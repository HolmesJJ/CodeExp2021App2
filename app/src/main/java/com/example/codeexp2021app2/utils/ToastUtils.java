package com.example.codeexp2021app2.utils;
...
public final class ToastUtils {
    ...
    public static void setView(@LayoutRes int layoutId) {
        ...
        LayoutInflater inflate = (LayoutInflater) ContextUtils.getContext().getSystemService
     ...
     * 显示自定义时长的toast
     */
    public static void showAutoTimeToast(@StringRes int resId, final int time) {

        showAutoTimeToast(ContextUtils.getContext().getResources().getString(resId), time);
    }

    /**
     * 显示自定义时长的toast
     */
    public static void showAutoTimeToast(final String text, final int time) {

        final Timer timer = new Timer();
        int longToastTime = 3000;
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        ...
        show(String.format(ContextUtils.getContext().getResources().getString(resId), args),
                duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                spannableString.setSpan(colorSpan, 0, spannableString.length(), Spanned
                        .SPAN_EXCLUSIVE_EXCLUSIVE);
                sToast = Toast.makeText(ContextUtils.getContext(), spannableString, duration);
            } else {
